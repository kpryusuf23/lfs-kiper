...
$SwearWordsList = "oç,orospu,çocuğu,OÇ,OROSPU,ÇOCUĞU";
...
Event OnSwearWords2( $userName )  # Player event
	privMsg( "^T^7AİLEVİ KÜFÜRLERE YER YOK^1!!!^61 GÜN BANLANDIN." );
	cmdLFS("/ban ".GetCurrentPlayerVar("UserName"). " 1");
EndEvent
...
CASE "!res":
		...
		CASE "!auth":
			...
			BREAK;
		CASE "!dc":
			privMsg("^T^4Discord ^7Adresimiz:^1https://discord.gg/EXDxYvzgRy");
			BREAK;
		CASE "!cleanspb":
			cleanSpb();
			BREAK;
		...
		CASE "!dstats":
			...
			BREAK;
		CASE "!sigara":
			privMsg( ( $userName ) . "^T^11 ^7Adet ^3SİGARA ^1İçtin");
			BREAK;
		CASE "!drfnear":
			drfNear( $argv );
			BREAK;
...
#{AccelerationStartSpeed2} - Starting speed
#{AccelerationEndSpeed2} - Ending speed
#{AccelerationTime2} - Acceleration time achieved from start to end speed
#{UnitSpeed} Unit of the Speed of the player connected

Event OnAcceleration2( $userName )  # Player event
	#privMsg( langEngine( "%{main_accel}%" , GetCurrentPlayerVar("AccelerationTime2") , GetCurrentPlayerVar("AccelerationEndSpeed2") , GetCurrentPlayerVar("UnitSpeed") ) );
EndEvent
...
Lang "EN"
	...
	main_ban = "{0} ^T^7Moderator Tarafından Ban Atılmıştır {1}";
	main_kick = "{0} ^T^7Moderator Tarafından Kick Atılmıştır {1}";
	...
	main_join_server = {0} ^T^7Sunucumuza ^1Hoşgeldin ^7İyi Oyunlar... ";
